...
from .serializers import (MessageListSerializer, MessageSerializers, ConversationDetailSerializers, ConversationListSerializer, GroupSerializers, GroupMessageSerializer, UserSerializers)
...
class CreateMessageView(BaseResponseView, CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return self.generate_response(status.HTTP_201_CREATED, "Message Sent", MessageSerializers(message).data)
...
class MessageListView(BaseResponseView, ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (Message.objects.filter(models.Q(sender=user, receiver=receiver) | models.Q(sender=receiver, receiver=user)).order_by("-timestamp"))
    
    def list(self, request, *args, **kwargs):
        queryset = self.get_queryset()
        serializer = self.get_serializer(queryset, many=True)
        
        return self.generate_response(status.HTTP_200_OK, "Messages Successfully retrieved", serializer.data)
...
class MessageDeleteView(BaseResponseView, DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return self.generate_response(status.HTTP_204_NO_CONTENT, "Message succsssfully deleted")
...
class MessageUpdateView(BaseResponseView, UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return self.generate_response(status.HTTP_200_OK, "Message successfully updated", serializer.data)
...
class ConversationListView(BaseResponseView, ListAPIView):
    ...
    def get_queryset(self):
        ...
        sent_messages = Message.objects.filter(sender=self.request.user).values_list("receiver", flat=True)
        received_messages = Message.objects.filter(receiver=self.request.user).values_list("sender", flat=True)
    ...
    def list(self, request, *args, **kwargs):
        ...
        return self.generate_response(status.HTTP_200_OK, "Conversations retrieved successfully", serializer.data)
...
class CreateGroupView(BaseResponseView, CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return self.generate_response(status.HTTP_201_CREATED, "Group Successfully Created", GroupSerializers(group).data)
...
class CreateGroupMessageView(BaseResponseView, CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        return serializer.save(sender=self.request.user, group=group)
    def create(self, request, *args, **kwargs):
        group_id = self.kwargs.get("group_id")
        ...
        return self.generate_response(status.HTTP_201_CREATED, "Group Message sent", GroupMessageSerializer(group_message).data)
